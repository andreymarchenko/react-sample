...
describe('Utils', () => {
    it('should not filter data by empty filter', () => {
        const filteredData = filterData(data);

        expect(filteredData).toEqual(data);
    });

    it('should filter data by filter with 1 condition', () => {
        const filteredData = filterData(data, { type: 'airplane' });

        expect(filteredData.length).toEqual(4);
    });

    it('should filter data by filter with 2 condition', () => {
        const filteredData = filterData(data, {
            type: 'airplane',
            color: 'red'
        });
    ...
    it('should filter data by filter with 3 condition', () => {
        const filteredData = filterData(data, {
            type: 'airplane',
            color: 'red',
            brand: 'Airbus A400M Atlas'
    ...
    it('should return empty data by non existing filter', () => {
        const filteredData = filterData(data, {
            color: 'pink'
        });
    ...
    it('should return unique items by property', () => {
        const uniqueOptions = getUniquePropertyValuesByFilter(data, 'type');

        expect(uniqueOptions.length).toEqual(3);
    });

    it('should return unique items by property and filter', () => {
        const uniqueOptions = getUniquePropertyValuesByFilter(data, 'type', {
            color: 'green'
        });

        expect(uniqueOptions.length).toEqual(2);
    });

    it('should return unique items for arrays', () => {
        const uniqueItems = getUniqueItemsForArrays(data, 'colors');

        expect(uniqueItems.length).toEqual(8);
    });
});
